def calcMaxGainsArray(a, k):
    gain_a_left = {0: 0, len(a): sum(a)}
    for num_ans_left_a in range(1, min(len(a), k)):
        gain_a_left[num_ans_left_a] = sum(a[:num_ans_left_a])
    gain_a_right = {0: 0, len(a): sum(a)}
    for num_ans_right_a in range(1, min(len(a), k)):
        gain_a_right[num_ans_right_a] = sum(a[len(a) - num_ans_right_a :])
    gains_a = {0: 0, len(a): sum(a)}
    for num_ans_a in range(1, min(len(a), k)):
        max_gain = 0
        for num_ans_a_left in range(0, num_ans_a + 1):
            num_ans_a_right = num_ans_a - num_ans_a_left
            gain = gain_a_left[num_ans_a_left] + gain_a_right[num_ans_a_right]
            if gain > max_gain:
                max_gain = gain
        gains_a[num_ans_a] = max_gain
    return gains_a
