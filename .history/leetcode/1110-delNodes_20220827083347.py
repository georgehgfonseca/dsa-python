...
class Solution:
    def delNodes(self, root: Optional[TreeNode], to_delete: List[int]) -> List[TreeNode]:
        ...
        def helper(root: Optional[TreeNode], del_node: List[int], ans: List[Optional[TreeNode]]) -> bool:
            if root.val == del_node:
                ...
                return True
            if root.left:
                helper(root.left, del_node, ans)
            if root.right:
                helper(root.right, del_node, ans)
            return False

        for i in range(len(to_delete)):
            # delete i from each tree in ans (probably only one)
            for t in range(len(ans)):
                if helper(ans[t], to_delete[i], ans):
                    break
